...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        $stmt = $conn->prepare("SELECT * FROM luckperms_players WHERE username = ?");
        if (!$stmt) {
        ...
        if (isset($response['error'])) {
            logError("Pterodactyl error: " . $response['error']);
        } else {
            logError("Pterodactyl command sent for $cleanName");
        }
